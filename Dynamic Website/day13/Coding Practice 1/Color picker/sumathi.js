function button1(){
    document.getElementById("colorPickerContainer").style.backgroundColor = "#e0e0e0";
    document.getElementById("colorPickerHeader").textContent = "#e0e0e0"

}
function button2(){
    document.getElementById("colorPickerContainer").style.backgroundColor = "#6fcf97";
    document.getElementById("colorPickerHeader").textContent = "#6fcf97"

}
function button3(){
    document.getElementById("colorPickerContainer").style.backgroundColor = "#56ccf2";
    document.getElementById("colorPickerHeader").textContent = "#56ccf2"

}
function button4(){
    document.getElementById("colorPickerContainer").style.backgroundColor = "#bb6bd9";
    document.getElementById("colorPickerHeader").textContent = "#bb6bd9"

}